...
void create_cell_buttons(map_t *map) {
    ...
    for (y = 0, cell = map->cells; y < map->height; y++) {
        for (x = 0; x < map->width; x++, cell++) {
            ...
            cell->widget = button;
            ei_button_configure(button, &g_button_size, &g_bg_color, &border_width, &corner_radius, &relief, NULL, NULL,
                                NULL, NULL, NULL, NULL, NULL, &callback, NULL);
...
void create_game_window(map_t *map) {
    ...
    ei_size_t toplevel_size = ei_size(g_button_size.width * map->width,
                                      k_info_height + g_button_size.height * map->height);
    ...
    ei_toplevel_configure(map->toplevel, &toplevel_size, &toplevel_bg, &border_width, &label_str_ptr, NULL, &resizable,
                          NULL);
